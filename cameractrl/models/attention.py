...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.cc_projection = zero_module(nn.Linear(cross_attention_dim + 96, cross_attention_dim))
    ...
    def forward(
        ...
        cross_tmp = norm_hidden_states[0][None]
        norm_hidden_states = rearrange(norm_hidden_states, "(b f) n c -> b (f n) c", f = batch_size)
        #
        ...
        #
        norm_hidden_states = rearrange(norm_hidden_states, " b (f n) c -> (b f) n c ", f=batch_size)
        cross_attn_output = rearrange(cross_attn_output, " b (f n) c -> (b f) n c ", f=batch_size)
        ...
        if self.attn2 is not None:
            ...
            encoder_hidden_states = encoder_hidden_states + self.cc_projection(torch.cat([encoder_hidden_states,camera_embeddings],dim = 2))
