...
class UNet3DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def __init__(
        ...
        self.epipolar_up = nn.ModuleList([])
        ...
        for i, up_block_type in enumerate(up_block_types):
            ...
            if self.num_epipolar_layers[i] !=0 and len(self.epipolar_up)<=1:
                tmp_epipolar = nn.ModuleList([])
                for _ in range(self.num_epipolar_layers[i]):
                    tmp_epipolar.append(EpipolarTransformerBlock(
                        attention_head_dim[i] * output_channel // attention_head_dim[i],
                        attention_head_dim[i],
                        output_channel // attention_head_dim[i],
                    ))
                ...
                tmp_epipolar = None
            self.epipolar_up.append(tmp_epipolar)
    ...
    def set_image_layer_lora(self, image_layer_lora_rank: int = 128):
        ...
        for name in self.attn_processors.keys():
            ...
            cross_attention_dim = None if (name.endswith("attn1.processor") or name.endswith("attn3.processor") )else self.config.cross_attention_dim
...
class UNet3DConditionModelPoseCond(UNet3DConditionModel):
    ...
    def set_all_attn_processor(self,
        ...
        if add_spatial:
            ...
            for name in self.attn_processors.keys():
                ...
                cross_attention_dim = None if (attention_name == 'attn1' or attention_name == 'attn3') else self.config.cross_attention_dim
                if name.startswith("mid_block"):
                    hidden_size = self.config.block_out_channels[-1]
                    block_id = -1
    ...
    def forward(
            ...
            camera_embeddings: Optional[torch.Tensor] = None,
            cross_attention_kwargs: Optional[Dict[str, Any]] = None,
        ...
        for i, (upsample_block, motion_module_alpha) in enumerate(zip(self.up_blocks, motion_module_alphas[:-1][::-1])):
            ...
            if self.epipolar_up[i]:
                for epipolar_block in self.epipolar_up[i]:
                    sample = epipolar_block(
                        sample,
                        attention_mask=attention_mask_epipolar[1-i] if attention_mask_epipolar is not None else None,
                        image_only_indicator=image_only_indicator,
                    )
