...
from cameractrl.models.transformer_2d import Transformer2DModel
...
class UNetMidBlock3DCrossAttn(nn.Module):
    ...
    def forward(self, hidden_states, temb=None, encoder_hidden_states=None, attention_mask=None,
                motion_module_alpha=1., camera_embeddings = None, cross_attention_kwargs=None, motion_cross_attention_kwargs=None):
        ...
        for attn, resnet, motion_module in zip(self.attentions, self.resnets[1:], self.motion_modules):
            frame_len = hidden_states.shape[2]
            hidden_states = rearrange(hidden_states, "b c f h w -> (b f) c h w")
            hidden_states = attn(hidden_states, encoder_hidden_states=encoder_hidden_states, camera_embeddings = camera_embeddings, frame_len = frame_len,
...
class CrossAttnDownBlock3D(nn.Module):
    ...
    def forward(self, hidden_states, temb=None, encoder_hidden_states=None, attention_mask=None,
                motion_module_alpha=1., camera_embeddings = None, cross_attention_kwargs={}, motion_cross_attention_kwargs={}):
        video_length = hidden_states.shape[2]
        temb_repeated = repeat(temb, "b c -> (b f) c", f=video_length)
        ...
        for resnet, attn, motion_module in zip(self.resnets, self.attentions, self.motion_modules):
            if self.training and self.gradient_checkpointing:
                ...
                hidden_states = rearrange(hidden_states, "b c f h w -> (b f) c h w")
                hidden_states = attn(hidden_states, encoder_hidden_states=encoder_hidden_states, camera_embeddings =camera_embeddings, frame_len = video_length,
                                     cross_attention_kwargs=cross_attention_kwargs).sample
                hidden_states = rearrange(hidden_states, "(b f) c h w -> b c f h w", f=video_length)
