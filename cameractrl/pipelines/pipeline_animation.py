            ...
            for i, t in enumerate(timesteps):
                ...
                for multidiff_step in range(multidiff_total_steps):
                    ...
                    latent_model_input = torch.cat(
                        [latent_partial] * 2) if do_classifier_free_guidance else latent_partial
                ...
                for pred_idx, noise_pred in enumerate(noise_preds):
                    ...
                    noise_pred_full[:, :, start_idx: start_idx + single_model_length] += noise_pred / mask_full[:, :,
...
class CameraCtrlPipeline(AnimationPipeline):
    ...
    def __init__(self,
        ...
        super().__init__(vae, text_encoder, image_encoder, feature_extractor, tokenizer, unet, scheduler)
    ...
    def __call__(
            ...
            eta: float = 0.0,
            generator: Optional[Union[torch.Generator, List[torch.Generator]]] = None,
            test_condition_image_latents:  Optional[torch.FloatTensor] = None, #1stframe
            test_image_embedding: Optional[torch.FloatTensor] = None,
            test_camera_embeddings: Optional[torch.FloatTensor] = None,
            latents: Optional[torch.FloatTensor] = None,
            output_type: Optional[str] = "tensor",
            return_dict: bool = True,
            callback: Optional[Callable[[int, int, torch.FloatTensor], None]] = None,
        ...
        test_image_embedding = test_image_embedding.repeat_interleave(2, 0)
        test_camera_embeddings = test_camera_embeddings.repeat_interleave(2, 0)
        text_embeddings = torch.cat([text_embeddings, test_image_embedding], dim=1)
        # text_embeddings = torch.zeros_like(text_embeddings)
        # text_embeddings = test_image_embedding
        ...
        if isinstance(pose_embedding, list):
            ...
            for pe in pose_embedding:
                ...
                pose_embedding_feature = [rearrange(x, '(b f) c h w -> b c f h w', b=bs) for x in
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for multidiff_step in range(multidiff_total_steps):
                    ...
                    # expand the latents if we are doing classifier free guidance
                    # print(latent_partial.shape)
                    # print(test_condition_image_latents.shape)
                    latent_partial[:,:,0] = test_condition_image_latents #1stframe
                    latent_model_input = torch.cat(
                        [latent_partial] * 2) if do_classifier_free_guidance else latent_partial  # [2b c f h w]
                    latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
                    # predict the noise residual
                    noise_pred = self.unet(
                        ...
                        camera_embeddings = test_camera_embeddings,
